...
{
    public class GetFavouriteClientPropertiesQueryHandler : IRequestHandler<GetFavouriteClientPropertiesQuery, Pagination<ClientPropertyDto>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly string _baseApiUrl;

        public GetFavouriteClientPropertiesQueryHandler(IUnitOfWork unitOfWork, IConfiguration configuration)
        {
            _unitOfWork = unitOfWork;
            _baseApiUrl = configuration["BaseApiUrl"] ?? string.Empty;
        }

        public async Task<Pagination<ClientPropertyDto>> Handle(GetFavouriteClientPropertiesQuery request, CancellationToken cancellationToken)
        {
            var spec = new ClientFavouritePropertiesSpecification(request.ClientId, request.Params);
            var countSpec = new ClientFavouritePropertiesCountSpecification(request.ClientId);
